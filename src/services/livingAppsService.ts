...
import type { Mitarbeiter, Taetigkeiten, Zeiterfassung, CreateMitarbeiter, CreateTaetigkeiten, CreateZeiterfassung } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- TAETIGKEITEN ---
  static async getTaetigkeiten(): Promise<Taetigkeiten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TAETIGKEITEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Taetigkeiten[];
    return enrichLookupFields(records, 'taetigkeiten');
  }
  static async getTaetigkeitenEntry(id: string): Promise<Taetigkeiten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TAETIGKEITEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Taetigkeiten;
    return enrichLookupFields([record], 'taetigkeiten')[0];
  }
  static async createTaetigkeitenEntry(fields: CreateTaetigkeiten) {
    return callApi('POST', `/apps/${APP_IDS.TAETIGKEITEN}/records`, { fields: cleanFieldsForApi(fields as any, 'taetigkeiten') });
  }
  static async updateTaetigkeitenEntry(id: string, fields: Partial<CreateTaetigkeiten>) {
    return callApi('PATCH', `/apps/${APP_IDS.TAETIGKEITEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'taetigkeiten') });
  }
  static async deleteTaetigkeitenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TAETIGKEITEN}/records/${id}`);
  }
  ...
  static async getZeiterfassung(): Promise<Zeiterfassung[]> {
    ...
    return enrichLookupFields(records, 'zeiterfassung');
  }
  static async getZeiterfassungEntry(id: string): Promise<Zeiterfassung | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Zeiterfassung;
    return enrichLookupFields([record], 'zeiterfassung')[0];
  }
  static async createZeiterfassungEntry(fields: CreateZeiterfassung) {
    return callApi('POST', `/apps/${APP_IDS.ZEITERFASSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'zeiterfassung') });
  }
  static async updateZeiterfassungEntry(id: string, fields: Partial<CreateZeiterfassung>) {
    return callApi('PATCH', `/apps/${APP_IDS.ZEITERFASSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'zeiterfassung') });
  }
