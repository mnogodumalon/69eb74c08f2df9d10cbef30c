...
import { TaetigkeitenViewDialog } from '@/components/dialogs/TaetigkeitenViewDialog';
...
const TAETIGKEITEN_FIELDS = [
  { key: 'taetigkeit_name', label: 'Bezeichnung der Tätigkeit', type: 'string/text' },
  { key: 'taetigkeit_beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'taetigkeit_kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'beratung', label: 'Beratung' }, { key: 'buchhaltung', label: 'Buchhaltung' }, { key: 'steuern', label: 'Steuern' }, { key: 'verwaltung', label: 'Verwaltung' }, { key: 'sonstiges', label: 'Sonstiges' }] },
];
const ZEITERFASSUNG_FIELDS = [
  ...
  { key: 'mandant', label: 'Mandant', type: 'lookup/select', options: [{ key: 'bag', label: 'BAG' }, { key: 'reichruthhard', label: 'ReichRuthhard' }, { key: 'juergen_reich', label: 'Jürgen Reich' }, { key: 'matthias_ruthhard', label: 'Matthias Ruthhard' }] },
  ...
  { key: 'taetigkeit_ref', label: 'Tätigkeit', type: 'applookup/select', targetEntity: 'taetigkeiten', targetAppId: 'TAETIGKEITEN', displayField: 'taetigkeit_name' },
  { key: 'notiz', label: 'Notiz / Beschreibung', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'mitarbeiter', label: 'Mitarbeiter', pascal: 'Mitarbeiter' },
  { key: 'taetigkeiten', label: 'Tätigkeiten', pascal: 'Taetigkeiten' },
  { key: 'zeiterfassung', label: 'Zeiterfassung', pascal: 'Zeiterfassung' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'taetigkeiten': return (data as any).taetigkeiten as Taetigkeiten[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'zeiterfassung':
        ...
        lists.taetigkeitenList = (data as any).taetigkeiten ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'zeiterfassung' && fieldKey === 'mitarbeiter_ref') {
      const match = (lists.mitarbeiterList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'zeiterfassung' && fieldKey === 'taetigkeit_ref') {
      const match = (lists.taetigkeitenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.taetigkeit_name ?? '—';
    }
    return String(url);
  }, [getLookupLists]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'taetigkeiten': return TAETIGKEITEN_FIELDS;
      case 'zeiterfassung': return ZEITERFASSUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'taetigkeiten': return {
        create: (fields: any) => LivingAppsService.createTaetigkeitenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTaetigkeitenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTaetigkeitenEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'taetigkeiten' || dialogState?.entity === 'taetigkeiten') && (
        <TaetigkeitenDialog
          open={createEntity === 'taetigkeiten' || dialogState?.entity === 'taetigkeiten'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'taetigkeiten' ? handleUpdate : (fields: any) => handleCreate('taetigkeiten', fields)}
          defaultValues={dialogState?.entity === 'taetigkeiten' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'zeiterfassung' || dialogState?.entity === 'zeiterfassung') && (
        <ZeiterfassungDialog
          ...
          taetigkeitenList={(data as any).taetigkeiten ?? []}
      ...
      {viewState?.entity === 'taetigkeiten' && (
        <TaetigkeitenViewDialog
          open={viewState?.entity === 'taetigkeiten'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'taetigkeiten', record: r }); }}
        />
      )}
